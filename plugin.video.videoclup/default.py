# -*- coding: utf-8 -*-
# modificado por xkiler para proyecto solocine
import urllib
import urllib2
import datetime
import re
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.SoloCine Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            addon_log('URL: '+url)
            if hasattr(e, 'code'):
                addon_log('Something Went Wrong  - %s.' % e.code)
                xbmc.executebuiltin("XBMC.Notification(SoloCine,Something Went Wrong  - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(SoloCine,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    addon_log("SKindex")
    addDir('[COLOR green]---------------------------------------------------------------------[/COLOR]','favorites',4,'' ,  FANART,'','','','')
    getData(_Edit.MainBase,'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
#   addDir COLOCAR URL,FANART,/ICONS/',FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(SoloCine,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('SoloCine Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('SoloCine Searching Please wait',' ')
                        ...
                        for i in item('f4m'):
                            if not i.string == None:
                                if '.f4m' in i.string:
                                    ...
                                    f4m = 'plugin://plugin.video.f4mTester/?streamtype=HLSRETRY&amp;name='+urllib.quote_plus(i.string)+''
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(SoloCine,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(SoloCine,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('SoloCine','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('SoloCine', 'Do you want to add this file as a source?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(SoloCine,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if not 'f4m'in url:
        if '.m3u8' in url:
            url = 'plugin://plugin.video.f4mTester/?streamtype=HLSRETRY&amp;name='+url+'&amp;url='+url	
        elif '.ts'in url:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(SoloCine ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(SoloCine,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
