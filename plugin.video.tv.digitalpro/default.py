...
def SportsDevil():
    print "SportsDevil"
    url = "http://repo.tv.digitalpro.org/plugin.video.SportsDevil-2016-03-06.zip"
...
addon = xbmcaddon.Addon('plugin.video.tv.digitalpro')
...
SOURCES = [{"url": "https://dl.dropbox.com/s/h0nzigp5y1v6n8r/tv.digital.pro.2.2.1.xml", "fanart": "http://i64.tinypic.com/2nsw6f7.jpg"}]


def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.tv.digitalpro-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(fabian,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(fabian,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
def getSources():
	try:
		if os.path.exists(favorites) == True:
			FAV = open(favorites).read()
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(fabian,New source added.,5000,"+icon+")")
        if not url is None:
